...
def returnFastestLegal(board, legalList):
    rows, cols = len(legalList), len(legalList[0])
    
    minLegalPosition=None
    smallestLegal=10
...
def legalBan(row, col, value, legalList):
    legalList[row][col].discard(value)
    return legalList
    
def solveSudoku(board):
    legals=getLegals(board)
    return solveSudokuHelper(copy.deepcopy(board), legals)

def solveSudokuHelper(board, legals):
    #legals=getLegals(board)
    smallestLegalPosition =returnFastestLegal(board, legals)
    if noZerosInBoard(board)==True:
        ...
        for num in legalSet:
            ...
            if isLegalSudoku(board):
                newLegals = setLegal(board, row, col, num, copy.deepcopy(legals))
                solution =solveSudokuHelper(copy.deepcopy(board), newLegals)
                if solution!=None:
                    return solution
            board[row][col]=0
            #newLegals=getLegals(board)
        return None
